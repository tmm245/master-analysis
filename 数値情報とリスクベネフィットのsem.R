...
# モデル情報（依存変数 dep は不要）
models_info <- list(
  list(grp = "berlin_oneItem", levels = NULL),
  list(grp = "berlin_group", levels = c("BL", "BH")),
  list(grp = "numAttitude_group", levels = NULL),
  list(grp = "subjectiveNumeracy_group", levels = NULL)
)
...
for (i in seq_along(models_info)) {
  ...
  path_tests <- lapply(paths_to_test, function(path_label) {
    fit_partial <- sem(model_syntax, data = subset_data, group = grp,
                       group.equal = "regressions", group.partial = path_label)
...
write.csv(filtered_results, "selected_simpleModels_from_final_results.csv", row.names = FALSE)
